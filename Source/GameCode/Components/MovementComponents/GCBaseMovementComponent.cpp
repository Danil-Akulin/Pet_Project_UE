...
#include "GCBaseMovementComponent.h"


void UGCBaseMovementComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	Velocity += VerticalVelocity;         // Velocity = Velocity + VerticalVelocity
...
void UGCBaseMovementComponent::JumpStart()
{
	if (bCanJump)
	{
		bCanJump = false;
		...
		GetWorld()->GetTimerManager().SetTimer(JumpCooldownTimerHandle, [this]() {
			bCanJump = true;
			}, JumpCooldown, false);
	}
}

bool UGCBaseMovementComponent::IsFalling() const
{
	return bIsFalling;
}
